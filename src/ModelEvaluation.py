...
def test_model(test_data, base_model, calibrated_model, scalar, model_name, features):
    
    print(f'Evaluating {model_name}')
    
        
    X_test = test_data[features]
    y_true = test_data[config.TARGET]
    ...
    print(f"Baseline Fill percentage is {y_true.mean()*100:.2f} %")
    ...
    axes[0].plot(engine_prob_pred, engine_true, color = 'b' ,label = f'{model_name}')
    ...
    axes[1].plot(engine_fpr, engine_tpr, color = 'b', label = f'{model_name}')
    ...
    axes[2].plot(engine_recall, engine_precision, color = 'b', label = f'{model_name} PR')
