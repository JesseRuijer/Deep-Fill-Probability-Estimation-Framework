...
"""
Script to use Optuna to find best hyperparameters for LR
"""
...
def run_optuna() -> None:
    ...
    test_files = paths.get('test_bin', []) #This is just for selecting the val file, once done with optuna, can chagne the test file back to another day in main 
    val_file = test_files[0] if isinstance(test_files, list) else test_files
    train_frames = []
    ...
    train_matrix = train_matrix.sample(frac = 0.1, random_state = config.RANDOM_SEED) #During training the data was never sliced like this due to the timeseries timedependent nature of it. However for hyperparametrization this was deemed acceptible to sample different data points to use in the different trials
    ...
    def objective(trial) -> float:
        ...
        """
        Using Optuna to hyperparameter optimise LR
        """
        ...
        #Train
        model, _, scalar = train_logistic_model(X_train, y_train, weights_train, None, None, None, params = params, for_tuning = True)
